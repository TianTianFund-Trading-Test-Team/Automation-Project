...
class Test_JJZH_Sub():
    ...
    @allure.story('组合内基金超级转换 /Business/home/OperateFundTransfer')
    # 组合内基金超级转换
    def test_Business_Home_OperateFundTransfer(self):
        if read_yaml3() is None:
            pytest.skip(), '没有HH基金份额或者份额不足无法发起超级转换'
        else:
            url = urljoin(read_yaml1()[read_yaml4()["Env"]], "/Business/Home/OperateFundTransfer")
            datas = {
                ...
                "FldParam": "",
                "Uid": "",
                "FromFundCode": read_yaml3()["FundCode"],
                "ToFundCode": read_yaml3()["FundCode1"],
                "ShareId": read_yaml3()["ShareId"],
                "TransferVol": read_yaml3()["FundAmount_CJZH"],
                "LargeRedemptionFlag": "1",
                "TraceID": read_yaml3()["TraceID"],
                "TraceTime": "",
                "FundRisk": "",
                "CustomerRisk": "",
                "FundCode": "{{InFundCode_TF}}",
...
@allure.feature('组合内基金转换 36 免密')
class Test_JJZH_Sub_NP():
    ...
    @allure.story('组合内基金超级转换 /Business/home/OperateFundTransfer')
    # 组合内基金超级转换
    def test_Business_Home_OperateFundTransferNP(self):
        if read_yaml3() is None:
            pytest.skip(), '没有HH基金份额或者份额不足无法发起超级转换'
        else:
            url = urljoin(read_yaml1()[read_yaml4()["Env"]], "/Business/Home/OperateFundTransferNP")
            datas = {
                "UserId": read_yaml2()["CustomerNo"],
                "ShareID": read_yaml3()["ShareId"],
                "FldParam": "",
